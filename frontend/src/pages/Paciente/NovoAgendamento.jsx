...
import { 
    ...
    Building2, User, Car, LocateFixed, ArrowRight, CreditCard, QrCode, Lock, Wifi, Wheelchair
} from 'lucide-react';
...
// Função Matemática para Calcular Distância Real (Fórmula de Haversine)
const calcularDistancia = (lat1, lon1, lat2, lon2) => {
    ...
    return (R * c).toFixed(1); // Retorna com 1 casa decimal (Ex: 2.4)
};
...
const NovoAgendamento = () => {
    ...
    // 1. CARREGA PROFISSIONAIS E O ENDEREÇO DO PACIENTE
    useEffect(() => {
        const load = async () => {
            setLoading(true);
            try {
                const [resProf, resEnd] = await Promise.all([
                    ...
                    api.get('/agendamentos/meu-endereco') // <- Rota que criamos no backend
                ]);
                setProfissionais(resProf.data);
                setProfissionaisFiltrados(resProf.data);

                // Converte o endereço do paciente em coordenadas para calcular a distância depois
                ...
                if (pacEnd && pacEnd.rua && pacEnd.cidade) {
                    ...
                    if (geoData.length > 0) setPacienteLatLng({ lat: geoData[0].lat, lng: geoData[0].lon });
                }
    ...
    // 2. BUSCA AS CLÍNICAS DO MÉDICO ESCOLHIDO E CALCULA AS COORDENADAS
    const selectMedico = async (id, nome, valor) => {
        ...
        try {
            ...
            for (const c of clinicasObtidas) {
                ...
                // Calcula a distância real se tivermos as coordenadas do paciente e da clínica
            ...
            const primeira = clinicasComMapaEDistancia.find(c => c.latitude);
            if (primeira) setMapCenter([parseFloat(primeira.latitude), parseFloat(primeira.longitude)]);
    ...
    const modernInputStyle = {
        '& .MuiOutlinedInput-root': {
            borderRadius: '12px', bgcolor: '#F8FAFC', transition: 'all 0.2s ease-in-out',
                        ...
                        <Box sx={{ width: { xs: '100%', md: 460 }, borderRight: '1px solid #F1F5F9', bgcolor: '#FFF', display: 'flex', flexDirection: 'column', zIndex: 5, boxShadow: '20px 0 40px -20px rgba(0,0,0,0.05)' }}>
                            ...
                            <Box sx={{ flex: 1, overflowY: 'auto' }}>
                                {clinicas.map((c, i) => {
                                    ...
                                    // Pega as comodidades separadas por vírgula (ex: "Vagas, Wi-Fi")
                                    ...
                                    return (
                                        ...
                                        >
                                            <Stack direction="row" spacing={3} alignItems="center">
                                                <Avatar src={c.foto_perfil || c.logo} variant="rounded" sx={{ width: 64, height: 64, borderRadius: '16px', bgcolor: '#F1F5F9', color: '#94A3B8', border: '1px solid #E2E8F0' }}>
                                                    {!(c.foto_perfil || c.logo) && <Building2 size={32} strokeWidth={1.5} />}
                                                </Avatar>
                                                ...
                                                <Box sx={{ flex: 1 }}>
                                                    ...
                                                    {/* TAGS DINÂMICAS: Distância + Comodidades reais */}
                                                    <Stack direction="row" spacing={1.5} alignItems="center" flexWrap="wrap" useFlexGap sx={{ gap: '8px 0' }}>
                                                        {c.distancia && (
                                                            <Chip label={`${c.distancia} km`} size="small" icon={<LocateFixed size={12}/>} sx={{ fontWeight: 800, bgcolor: '#F1F5F9', color: '#64748B', borderRadius: '8px', height: 24, fontSize: '0.7rem' }} />
                                                        )}
