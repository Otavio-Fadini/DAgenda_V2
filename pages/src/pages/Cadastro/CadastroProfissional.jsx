...
import { Mail, Lock, User, Stethoscope, ClipboardList, Camera } from 'lucide-react';
import axios from 'axios';
import { useNavigate } from 'react-router-dom';
import logo from '../../assets/logo.png';

const CadastroProfissional = () => {
    const [formData, setFormData] = useState({ nome: '', registro: '', especialidade: '', email: '', senha: '', foto: null });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.post('http://localhost:3001/api/auth/cadastro-profissional', data);
            navigate('/');
                ...
                <form onSubmit={handleSubmit}>
                    <Box sx={{ display: 'flex', justifyContent: 'center', mb: 5, position: 'relative' }}>
                        <Avatar src={preview} sx={{ 
                            ...
                            <Stethoscope size={45} color="#cbd5e1" />
                        </Avatar>
                        ...
                        <label htmlFor="foto-profissional">
                            <IconButton component="span" sx={{ 
                                ...
                                '&:hover': { bgcolor: '#32B5FE' } // Hover com o novo azul
                            }}>
                                <Camera size={16} />
                            </IconButton>
                        </label>
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12}>
                            <TextField fullWidth label="CRM / Registro" variant="filled" InputProps={{ disableUnderline: true, startAdornment: <InputAdornment position="start"><Stethoscope size={18} color="#64748b"/></InputAdornment>, sx: { borderRadius: 3, bgcolor: '#f1f5f9' } }} onChange={(e) => setFormData({...formData, registro: e.target.value})} />
                        </Grid>
                        <Grid item xs={12}>
                            <TextField fullWidth label="Especialidade" variant="filled" InputProps={{ disableUnderline: true, startAdornment: <InputAdornment position="start"><ClipboardList size={18} color="#64748b"/></InputAdornment>, sx: { borderRadius: 3, bgcolor: '#f1f5f9' } }} onChange={(e) => setFormData({...formData, especialidade: e.target.value})} />
                        </Grid>
                        <Grid item xs={12}>
                            <TextField fullWidth label="E-mail Profissional" variant="filled" InputProps={{ disableUnderline: true, startAdornment: <InputAdornment position="start"><Mail size={18} color="#64748b"/></InputAdornment>, sx: { borderRadius: 3, bgcolor: '#f1f5f9' } }} onChange={(e) => setFormData({...formData, email: e.target.value})} />
                        </Grid>
                        <Grid item xs={12}>
                    ...
                    <Button 
                        ...
                        sx={{ 
                            ...
                            textTransform: 'none',
                            fontSize: '1.1rem',
                            '&:hover': { bgcolor: '#32B5FE', color: 'white' } // Hover azul com texto branco
                        }}
                    >
